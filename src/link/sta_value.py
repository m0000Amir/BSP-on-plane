"""
It is used link budget equation to calculate:
 - link distance parameter between station;
...
def get_value(gateway, user_device, sta):
    ...
    i = [_ for _ in range(len(sta_param.p_tr))]
    j = [_ for _ in range(len(sta_param.p_tr))]
    ...
    for s1, s2 in product(i, j):
        if s1 != s2:
            ld_input = GetDistanceInput(p_tr=sta_param.p_tr[s1],
                                        l_tr=sta_param.l[s1],
                                        g_tr=sta_param.g[s1],
                                        p_recv=sta_param.p_recv[s2],
                                        g_recv=sta_param.g[s2],
                                        l_recv=sta_param.l[s2])
            link_distance[s1, s2] = get_distance(ld_input, som=20)
    for s1 in i:
        ld2gtw_input = GetDistanceInput(p_tr=sta_param.p_tr[s1],
                                        l_tr=sta_param.l[s1],
                                        g_tr=sta_param.g[s1],
    ...
    for s1 in i:
        gtw2ld_input = GetDistanceInput(p_tr=gtw_param.p_tr,
                                        g_tr=gtw_param.g,
                                        l_tr=gtw_param.l,
                                        p_recv=sta_param.p_recv[s1],
                                        l_recv=sta_param.l[s1],
                                        g_recv=sta_param.g[s1],
                                        )
